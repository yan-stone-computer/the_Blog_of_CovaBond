...
class BlogBuilder:
    ...
    def generate_post_card_html(self, post):
        """生成单篇文章卡片的HTML"""
    ...
    def generate_blog_post_item_html(self, post):
        """生成博客页面文章项的HTML"""
        category_slug = self.get_category_slug(post['category'])
        return f'''        <article class="blog-post-item" data-category="{category_slug}">
    ...
    def process_index_html(self):
        ...
        posts_html = '\n'.join([self.generate_post_card_html(post) for post in self.posts_data[:3]])
        
        if '<p class="no-posts-message">暂无文章，请先添加文章</p>' in content:
            content = content.replace(
                '<p class="no-posts-message">暂无文章，请先添加文章</p>',
                posts_html
            )
        else:
            import re
            pattern = r'(<div class="posts-grid">)(.*?)(</div>)'
    ...
    def process_blog_html(self):
        ...
        posts_html = '\n'.join([self.generate_blog_post_item_html(post) for post in self.posts_data])
        ...
        with open(Path("dist/blog.html"), 'w', encoding='utf-8') as f:
            f.write(content)
